      ...
      <div className="flex-1 px-4 pt-4 pb-24">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <Link href="/feed" className="inline-block mt-5 bg-violet-600 text-white px-6 py-2.5 rounded-full font-semibold text-sm">
              Browse Feed
            </Link>
          </div>
        ) : (
            ...
            {conversations.map(c => (
              <Link
                ...
                className={`flex items-center gap-3 rounded-2xl p-4 transition-shadow ${
                  c.unread ? 'bg-violet-50 border border-violet-100' : 'bg-white shadow-sm'
                }`}
              >
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-violet-400 to-pink-400 flex items-center justify-center text-white font-black text-lg flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-violet-500 font-semibold truncate mb-0.5">{c.itemTitle}</p>
                  <p className={`text-xs truncate ${c.unread ? 'text-gray-800 font-semibold' : 'text-gray-400'}`}>
                    {c.lastMessage}
                  </p>
                </div>
                {c.unread && <div className="w-2.5 h-2.5 bg-violet-500 rounded-full flex-shrink-0" />}
              </Link>
            ))}
          </div>
        )}
