...
export default function FollowingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] flex flex-col">
      ...
      <h1 className="text-white font-black text-xl px-5 pb-4 tracking-tight">Following</h1>
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 text-center gap-3">
          <p className="text-white font-black text-lg">Not following anyone yet</p>
          <p className="text-white/40 text-sm">Find people to follow on the search page.</p>
          <Link href="/search" className="btn-halo mt-2">Find people</Link>
        </div>
      ) : (
          ...
          {people.map(p => (
            <div key={p.id} className="flex items-center gap-3">
              ...
              {clerkUser && p.id !== clerkUser.id && (
                <FollowButton targetUserId={p.id} initialFollowing={true} />
              )}
            </div>
          ))}
        </div>
